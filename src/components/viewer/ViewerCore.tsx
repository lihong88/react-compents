...
export default (props: ViewerProps) => {
  ...
  const [state, dispatch] = React.useReducer<(s: any, a: any) => ViewerCoreState>(
    reducer,
    initialState,
  );
  ...
  function loadImg(currentActiveIndex: any, isReset = false) {
    ...
    let activeImage: ImageDecorator | null = null;
    if (images.length > 0) {
  ...
  function getActiveImage(activeIndex2: any = undefined) {
  ...
  function handleZoom(targetX: any, targetY: any, direct: any, scale: any) {
    let imgCenterXY = getImageCenterXY();
